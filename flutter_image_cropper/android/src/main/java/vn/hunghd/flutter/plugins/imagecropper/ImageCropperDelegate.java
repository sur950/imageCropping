...
public class ImageCropperDelegate implements PluginRegistry.ActivityResultListener {
    ...
    public void startCrop(MethodCall call, MethodChannel.Result result) {
        ...
        Long cropFrameColor = call.argument("crop_frame_color");
        Long cropGridColor = call.argument("crop_grid_color");
        Long activeWidgetColor = call.argument("active_widget_color");
        Long dimmedLayerColor = call.argument("dimmed_layer_color");
        Long logoColor = call.argument("logo_color");
        methodCall = call;
        ...
        if (cropFrameColor != null) {
            int intColor = cropFrameColor.intValue();
            options.setCropFrameColor(intColor);
        }
        if (cropGridColor != null) {
            int intColor = cropGridColor.intValue();
            options.setCropGridColor(intColor);
        }
        if (activeWidgetColor != null) {
            int intColor = activeWidgetColor.intValue();
            options.setActiveWidgetColor(intColor);
        }
        if (dimmedLayerColor != null) {
            int intColor = dimmedLayerColor.intValue();
            options.setDimmedLayerColor(intColor);
        }
        if (logoColor != null) {
            int intColor = logoColor.intValue();
            options.setLogoColor(intColor);
        }
